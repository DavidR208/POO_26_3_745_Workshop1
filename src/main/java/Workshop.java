...
public class Workshop {
	...
	int[] fib = new int[n]; //Arreglo para n
	fib[0] = 0;
	if (n == 1) {
        	return fib;
        }
	...
	// Evita que el arreglo sea nulo o vacio
	...
	for (int i = 1; i < arreglo.length; i++) {
        	if (arreglo[i] > mayor) {
            		mayor = arreglo[i]; // Se registra nuev numero mayor
        	}
    	}
	return mayor;
    }
	...
	for (int numero : arreglo) {
		// Si se encuentra el elemento se hace directamente el true
		if (numero == elemento) {
            		return true;
		}
	}
        return False; // Si termina el bucle y no encontro el elemento 
    }
	...
	//Validar si es nulo o vacio el arreglo
	if (arreglo == null || arreglo.length == 0) {
        	return new int[0];
    	}

	int n = arreglo.length;
	// Comprobar  cuantos elementos unicos hay en el arreglo
	int cantidadUnicos = 0;
    	for (int i = 0; i < n; i++) {
        	boolean yaExiste = false;
		// ver si se repite [i] ya habia aprecido en el arreglo
		for (int j = 0; j < i; j++) {
		...
		// Si no estaba antes es un numero unico
		if (!yaExiste) {
            		cantidadUnicos++;
        	}
    	}
	//Nuevo arreglo sin repiticiones
	int[] resultado = new int[cantidadUnicos];
    	int indiceResultado = 0;
	for (int i = 0; i < n; i++) { // Copiar el primer numero del arreglo anterior
        	boolean yaExiste = false;
		for (int j = 0; j < i; j++) {	//Verifica si ya se habia agregado antes
            		if (arreglo[i] == arreglo[j]) {
                		yaExiste = true;
                		break;
            		}
	...
	//En caso de que alguno de los arreglos sea nulo
	if (arreglo1 == null) arreglo1 = new int[0];
   	if (arreglo2 == null) arreglo2 = new int[0];

    	int len1 = arreglo1.length;
    	int len2 = arreglo2.length;

	int[] resultado = new int[len1 + len2]; //Nuevo arreglo
	//Copiar los datos de los arreglos
	for (int i = 0; i < len1; i++) {
        	resultado[i] = arreglo1[i];
    	}
	for (int i = 0; i < len2; i++) {
        	resultado[len1 + i] = arreglo2[i];
    	}
	...
	for (int i = cadena.length() - 1; i >= 0; i--) {
        	invertida += cadena.charAt(i); // Concatenar carácter por carácter
    	}
        return invertida;
    }
	...
	//Ver caracteres de los extremos hacia el centro
	while (inicio < fin) {
	...
	for (Integer numero : lista) {
        	if (numero != null) { // Protección en caso de que la lista contenga nulos
            		suma += numero;
        	}
    	}
